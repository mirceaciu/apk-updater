...
/**
 * Returns the next apk version for provided name after the specified version.
 * This enables incremental updates.
 * @param {@String} name name (prefix) for apk files to look for.
 * @param {@Number} currentVersion the current version to find the next one for.
 */
function getNextVersion(name, currentVersion) {
  if (!initialized) { init(); }
  const versions = apk[name];
  if (!versions) return null;

  // Find the next available version
  return _.find(versions, function(def) {
    return def.version > currentVersion;
  });
}
