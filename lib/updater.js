...
androidUpdate.incremental_updater = function (req, res) {
  var name = req.body.pkgname,
      version = parseInt(req.body.version), // Ensure version is treated as an integer
      nextVersion = reader.getNextVersion(name, version); // Find the next version after current

  if (nextVersion) {
    const key = name + "-" + nextVersion.version;

    if (!links[key]) {
      links[key] = {
        url: routePfx + '/' + uuid.v4(),
        timeoutId: setTimeout(function() {
          for (let idx in expressApp.routes.get) {
            if (expressApp.routes.get[idx].path === links[key].url) {
              expressApp.routes.get.splice(idx, 1);
              break;
            }
      ...
      expressApp.get(links[key].url, function(req, res) {
        res.download(nextVersion.filepath);
      });

      // Move the new route to the beginning
      expressApp.routes.get.unshift(expressApp.routes.get.pop());
    }

    logger.info(`${getHumanReadableDate()}: Incremental Update for version ${version} to ${nextVersion.version} | ${links[key].url}`);
    res.send(`have update\n${links[key].url}\n${nextVersion.version}`);
  } else {
    logger.debug(`${getHumanReadableDate()}: No direct or incremental update for ${name}-${version}`);
    res.sendStatus(200);
  }
};
...
function enable(app, route, repoDir){
  ...
  app.post(route, androidUpdate.incremental_updater);
