...
androidUpdate.incremental_updater = function (req, res) {
  ...
  if (nextVersion) {
    ...
    logger.info(`${getHumanReadableDate()}: Update for version ${version} to ${nextVersion.version} | ${links[key].url}`);
    res.send(`have update\n${links[key].url}\n${nextVersion.version}`);
  } else {
    logger.debug(`${getHumanReadableDate()}: No update for ${name}-${version}`);
    res.sendStatus(200);
  }
};
